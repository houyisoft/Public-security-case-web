...
<template>
    <div class>
        <object ref="master" type="application/x-eloamplugin" width="305" height="270" name="view"></object>
        <object ref="slave" type="application/x-eloamplugin" width="305" height="270" name="view"></object>
        <object ref="thumb" type="application/x-eloamplugin" width="1208" height="150" name="thumb"></object>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        OpenVideoMain() {
            ...
            VideoMain = plugin().Device_CreateVideo(
                DeviceMain,
                nResolution,
                SelectType
            );
        ...
        OpenVideoAssist() {
            ...
            VideoAssist = plugin().Device_CreateVideo(
                DeviceAssist,
                nResolution,
                SelectType
            );
        ...
        changesubTypeMain() {
            if (DeviceMain) {
                ...
                if (sSubType.options.selectedIndex != -1) {
                    var txt =
                        sSubType.options[sSubType.options.selectedIndex].text;
                    if (txt == "YUY2") {
                        SelectType = 1;
                    } else if (txt == "MJPG") {
                        SelectType = 2;
                ...
                var nResolution = plugin().Device_GetResolutionCountEx(
                    DeviceMain,
                    SelectType
                );
                sResolution.options.length = 0;
                for (var i = 0; i < nResolution; i++) {
                    var width = plugin().Device_GetResolutionWidthEx(
                        DeviceMain,
                        SelectType,
                        i
                    );
                    var heigth = plugin().Device_GetResolutionHeightEx(
                        DeviceMain,
                        SelectType,
                        i
                    );
                    sResolution.add(
                        new Option(width.toString() + "*" + heigth.toString())
                    );
                }
                    ...
                    if (1 == dbt) {
                        ...
                        if (1 == deviceType) {
                            ...
                            if (null == DeviceMain) {
                                ...
                                if (DeviceMain) {
                                    document.getElementById(
                                        "lab1"
                                    ).innerHTML = plugin().Device_GetFriendlyName(
                                        DeviceMain
                                    );

                                    var sSubType = document.getElementById(
                                        "subType1"
                                    );
                            ...
                            if (null == DeviceAssist) {
                                DeviceAssist = plugin().Global_CreateDevice(
                                    1,
                                    idx
                                );
                                if (DeviceAssist) {
        ...
        EnableDate(obj) {
            if (obj.checked) {
                ...
                var font;
                font = plugin().Global_CreateTypeface(
                    50,
                    50,
                    0,
                    0,
        ...
        OpenVerifyFacRect(obj) {
            if (!initFaceDetectSuccess) {
                ...
                if (VideoMain) {
                    ...
                    else plugin().Global_DisableFaceRectCrop(VideoMain);
                }
                if (VideoAssist) {
                    if (obj.checked)
                        plugin().Global_EnableFaceRectCrop(VideoAssist, 1);
                    else plugin().Global_DisableFaceRectCrop(VideoAssist);
                }
            }
        },
        changescansize() {
        ...
        Scan() {
            ...
            if (VideoAssist) {
                var imgList2 = plugin().Video_CreateImageList(
                    VideoAssist,
                    0,
                    0
                );
        ...
        UploadToHttpServer() {
            ...
            var http = plugin().Global_CreateHttp(
                "http://192.168.1.56:8080/FileStreamDemo/servlet/FileSteamUpload?"
            ); //java服务器demo地址
            if (http) {
                var b = plugin().Http_UploadImageFile(
                    http,
                    "C:\\1.jpg",
                    "2.jpg"
                );
        ...
        ScanToHttpServer() {
            if (VideoMain) {
                ...
                if (img) {
                    ...
                    var http = plugin().Global_CreateHttp(
                        "http://192.168.1.56:80/FileStreamDemo/servlet/FileSteamUpload?"
                    ); //java服务器demo地址
                    if (http) {
                        var b = plugin().Http_UploadImage(
                            http,
                            img,
                            2,
                            0,
        ...
        StartMainRecord() {
            if (VideoMain) {
                ...
                if (
                    !plugin().VideoCap_PreCap(
                        videoCapMain,
                        "C:\\eloamPlugin_main.mp4",
                        selMicIdx,
                        FrameRate,
                        1,
                        videoOutputWidth,
        ...
        StopMainRecord() {
            if (plugin().VideoCap_Stop(videoCapMain)) {
                ...
                videoCapMain = null;
                alert(
                    "录像结束，文件保存于C:\\eloamPlugin_main.mp4\r\n若录像失败请选择较小的分辨率尝试"
                );
            } else {
                alert("停止录像失败");
            }
        ...
        StartAssistRecord() {
            if (VideoAssist) {
                ...
                if (
                    !plugin().VideoCap_AddVideoSrc(videoCapAssist, VideoAssist)
                ) {
                    alert("添加视频源失败");
                    return;
                }
        ...
        StopAssistRecord() {
            if (plugin().VideoCap_Stop(videoCapAssist)) {
                ...
                videoCapAssist = null;
                alert(
                    "录像结束，文件保存于C:\\eloamPlugin_assist.mp4\r\n若录像失败请选择较小的分辨率尝试"
                );
            } else {
                alert("停止录像失败");
            }
        ...
        Barcode() {
            if (DeviceMain) {
                if (VideoMain) {
                    var imgList = plugin().Video_CreateImageList(
                        VideoMain,
                        0,
                        0
                    );
                    if (imgList) {
                        ...
                        for (var i = 0; i < len; i++) {
                            ...
                            if (image) {
                                ...
                                if (b) {
                                    if (
                                        plugin().Global_GetBarcodeCount() <= 0
                                    ) {
                                        alert("识别失败！");
                                    } else {
                                        for (
                                            var i = 0;
                                            i <
                                            plugin().Global_GetBarcodeCount();
                                            i++
                                        )
                                            alert(
                                                plugin().Global_GetBarcodeData(
                                                    i
                                                )
                                            );
                                    }
        ...
        VerifyFaceDetect() {
            if (VideoAssist) {
                var image = plugin().Video_CreateImage(
                    VideoAssist,
                    0,
                    AssistView().View_GetObject()
                );
                var idcardImage = plugin().Global_CreateImageFromFile(
                    "C:\\idcard.jpg",
                    0
                );
    ...
    //生命周期 - 挂载完成（可以访问DOM元素）
    mounted() {
        VideoMain = this.plugin().Device_CreateVideo(
            DeviceMain,
            nResolution,
            SelectType
        );
