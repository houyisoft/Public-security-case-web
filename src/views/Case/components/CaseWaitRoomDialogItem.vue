...
@example 调用示例
<case-wait-room-dialog-item :personInfo="personInfo"
@onSavePersonInfo="savePersonInfo"></case-wait-room-dialog-item> -->
<template>
	<div class="wait-room-dialog-cpnt">
		<el-dialog :title="status == 1 ? '进入侯问室' : '离开侯问室'" :close-on-click-modal="false"
		 :visible="$store.state.Case.waitRoomDialog.visible"
		 @close="$store.commit('Case/updateWaitRoomDialog', {visible: false})">
			<el-row>
				<el-col class="dialog-left"
				 :span="12">
					<div class="left-title">
						<template v-if="status == 1">请进入侯问室的人员刷手环进入</template>
						<template v-if="status == 2">请离开侯问室的人员刷手环离开</template>
					</div>
...
<script>
	...
	export default {
		...
		computed: {
			// 进入/离开侯问室
			status() {
				return this.$store.state.Case.waitRoomDialog.status
			},
		},
		watch:{
		...
		methods: {
			savePersonInfo() {
				...
				this.$store.commit('Case/updateWaitRoomDialog', {
